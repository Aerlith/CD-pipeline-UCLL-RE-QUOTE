  ...
  class extends HTMLElement {
    ...
    connectedCallback() {
      this._shadowRoot
        ...
        .addEventListener("click", () => this.changeLang("nl"));
      this._shadowRoot
        .getElementById("lang-en")
        .addEventListener("click", () => this.changeLang("en"));
    ...
    updateText() {
      ...
      this._shadowRoot.getElementById("pending-title").textContent =
        t("pendingQuotes");
      this._shadowRoot.getElementById("approved-title").textContent = t(
        "recentApprovedQuotes",
      );
      this._shadowRoot.getElementById("count-title").textContent =
        t("totalQuotes");
      this._shadowRoot.getElementById("sum-title").textContent =
        t("totalValue");
      this._shadowRoot.getElementById("avg-title").textContent =
        t("averageValue");
    }
    ...
    render() {
      ...
      const pendingQuotes = this._quotes.filter(
        (q) => q.getStatus() === "Pending",
      );
      ...
      const approvedQuotes = this._quotes.filter(
        (q) => q.getStatus() === "Recently approved",
      );
      ...
      // Average
      const average =
        this._quotes.length > 0 ? sumOfMoney / this._quotes.length : 0;
