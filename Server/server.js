...
APP.put('/api/customers/:id', async (req, res) => {
    ...
    if (!name || !email || !address || !address.street || !address.streetNumber || !address.zipCode || !address.city) {
        return res.status(400).json({ error: 'Missing required fields' });
    }
...
APP.get('/api/quotes', async (req, res) => {
    try {
        ...
        const result = await quoteCollection.view('quotes', 'all', { include_docs: true });
...
APP.post('/api/save-offerte', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Kon offerte niet opslaan', details: error.message });
    }
});
